...
const PRICING = {
  product: "Real Estate Stickers (Rectangular)",
  currency: "AUD",
  sizes: [
    { name: "Small (600×200mm)", widthMm: 600, heightMm: 200, price: 6.0 },
    { name: "Medium (720×240mm)", widthMm: 720, heightMm: 240, price: 9.0 },
    { name: "Large (900×300mm)", widthMm: 900, heightMm: 300, price: 13.0 },
    { name: "Extra Large (1200×400mm)", widthMm: 1200, heightMm: 400, price: 18.0 },
  ],
...
function priceRealEstate({ sizeIndex, qty }: { sizeIndex: number; qty: number }) {
  ...
  return {
    size: { name: size.name, widthMm: size.widthMm, heightMm: size.heightMm },
    qty: q,
...
export default function StickerCalculator() {
  ...
  const getPreviewImage = (combination: string, text: string) => {
    if (text !== "SOLD") return null

    switch (combination) {
      case "white-blue":
        return "/S-Wht-bg_blue-text.png"
      case "black-white":
        return "/S-Black-bg_White-text.png"
      case "white-red":
        return "/S-White-bg_red-text.png"
      case "red-white":
        return "/S-Red-bg_white-text.png"
      case "blue-white":
        return "/MOCKUP-STICKER-custom-shape.png"
      case "white-black":
        return "/S-Wht-bg_bllack-text.png"
      default:
        return null
    }
  }
        ...
        <form className="space-y-3 sm:space-y-6">
          ...
          {selectedSize !== null &&
            ...
            (() => {
              ...
              const previewImage = getPreviewImage(selectedColorCombination, selectedText)
                  ...
                  <div className="border border-gray-300 rounded-lg p-4 bg-gray-50 flex justify-center">
                    {previewImage ? (
                      <img
                        ...
                        alt={`${selectedText} sticker preview`}
                        className="w-[120px] h-[120px] object-contain"
                      />
                    ) : (
                      <div
                        className="rounded-md shadow-sm border-2 flex items-center justify-center font-bold text-center text-3xl sm:text-4xl"
                        style={{
